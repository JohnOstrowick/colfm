...
static QIcon tintedIconFor(const QFileInfo &fi, int box) {
    QPixmap base = QFileIconProvider().icon(fi).pixmap(box);
    if (base.isNull()) return QIcon(QPixmap(box, box));
...
public:
    ...
    void rebuild() {
        ...
        QDir d(desktopDir());
        d.setFilter(QDir::AllEntries | QDir::NoDotAndDotDot | QDir::Readable);
        d.setSorting(QDir::Name | QDir::IgnoreCase);
        QFileInfoList list = d.entryInfoList();
        QList<QStandardItem*> rows; rows.reserve(list.size());
        for (const QFileInfo &fi : list) {
            QIcon ico = tintedIconFor(fi, 48);
...
inline void ColFM::drawDesktopWindow() {
//QMessageBox::information(nullptr, "Debug", "drawDesktopWindow() called");
    ...
    view->setGeometry(w->rect().adjusted(16,16,-80,-16));
    view->show();
    view->raise();
