...
// -------------------- ContextMenu utility namespace --------------------

namespace ContextMenu {

inline void addLabelSwatchesToMenu(QMenu &menu,
                                   QWidget *parent,
    ...
    auto addSwatch = [&](const QString &name, const QString &hex) {
        ...
        QObject::connect(tb, &QToolButton::clicked, &menu, [parent, model, name, getCWD] {
            QString baseDir = getCWD();
            QStringList targets;
            ...
            if (auto *av = qobject_cast<QAbstractItemView*>(QApplication::focusWidget())) {
                if (av->selectionModel()) {
                    ...
                    for (const QModelIndex &ix : idxs) {
                        if (ix.column() == 0) {
                            const QString p = pathFromModelIndex(model, ix);
                            if (!p.isEmpty()) targets << p;
                        }
                    }
                }
            }
            if (targets.isEmpty()) targets << baseDir;

            for (const QString &p : targets) {
                const QFileInfo fi(p);
                const QString dir = fi.isDir() ? fi.absoluteFilePath() : fi.absolutePath();
                const QString itemName = fi.fileName().isEmpty() ? QStringLiteral(".") : fi.fileName();
                const QString labelFile = dir + QStringLiteral("/.labelcolor");
                ...
                if (f.open(QIODevice::Append | QIODevice::Text)) {
                    ...
                    QMessageBox::warning(parent, "Write failed",
    ...
    swatchAction->setDefaultWidget(row);
    menu.addSeparator();
...
inline void showContextMenu(QWidget *parent,
    ...
    if (path == QStringLiteral("trash://") || kind == QStringLiteral("Trash")) {
        ...
        ContextMenu::addLabelSwatchesToMenu(menu, parent, model, [&](){ return dir; });
        menu.exec(globalPos);
        return;
    }
...
//    QAction *actFolderize = menu.addAction("Move all to new folder");
menu.addAction(QStringLiteral("Move all to new folder"));
