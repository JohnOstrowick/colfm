...
tb->clear();
    ...
    tb->addSeparator();
    // group 5
    // breadcrumbs are a separate toolbar, no change here
    // TODO: add Search (Plocate) later
...
// Ensure Space and Ctrl+I always trigger preview (even if toolbar action doesn't get focus)
auto scSpace = new QShortcut(QKeySequence(Qt::Key_Space), this);
scSpace->setContext(Qt::ApplicationShortcut);
connect(scSpace, &QShortcut::activated, this, [this]{ auto i = currentIndex(); if (i.isValid()) previewFile(i); });

auto scInfo  = new QShortcut(QKeySequence(Qt::CTRL | Qt::Key_I), this);
scInfo->setContext(Qt::ApplicationShortcut);
connect(scInfo,  &QShortcut::activated, this, [this]{ auto i = currentIndex(); if (i.isValid()) previewFile(i); });
